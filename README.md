...
- `platform/` — platform implementation files (only Win32 for now)
...
Requires CMake 3.20+ and a C99 / C++20 toolchain. SDL3 and Dear ImGui are fetched automatically, so a fresh clone needs no setup:
...
Targets: `kek` (engine), `SnusShooter` (game logic), `kek_editor` (editor), and `SnusShooter_win32` (the playable executable, Windows only).
